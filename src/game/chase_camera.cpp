...
std::int32_t headingFromDirection(double x, double z) noexcept {
    ...
    return normalizeHeading(static_cast<std::int64_t>(std::lround(
        radians * static_cast<double>(heading_angle_units) /
        (2.0 * std::numbers::pi))));
}

HorizontalBasis headingBasis(std::int32_t heading) noexcept {
    const auto radians = static_cast<double>(normalizeHeading(heading)) *
...
CameraRay cameraRayAtProjectionOffset(
    ...
    const auto right = normalize(
        right_cross[0], right_cross[1], right_cross[2]);
    const auto down = cross(forward, right);
    const auto projection = std::max(1.0, std::abs(
        static_cast<double>(camera.projection)));
    const auto direction = normalize(
        forward[0] + right[0] * horizontal_offset / projection +
...
CameraState ChaseCamera::follow(
    double x,
    double y,
    double z,
...
CameraState FirstPersonCamera::view(
    double x,
    double y,
    double z,
    std::int32_t heading,
    ...
    const auto pitch_radians = pitch *
        (2.0 * std::numbers::pi / static_cast<double>(heading_angle_units));
