...
The build stages the required runtime DLLs beside the executable. Do not launch
from an intermediate directory that lacks those files.
...
The script stages the executable, exact runtime DLLs, license texts and
public-test documentation; rejects missing dependencies, saves, game images,
the cheat marker, generated localization data and developer artifacts; writes
per-file hashes; and produces a ZIP plus a `.zip.sha256` sidecar under `dist/`.
