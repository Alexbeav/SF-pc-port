...
typedef struct
{
	...
	VERTTYPE x, y, z;	 // 16 bit values (for lookup and backwards compat if not
						 // found in cache)
} PGXPVector3D;
...
extern "C"
{
	...
	int PGXP_GetCacheData(PGXPVData* out, uint lookup,
						  ushort indexhint /* = 0xFFFF */);

	/* retrieves the exact GTE vertex recorded at an addPrim-relative index */
	int PGXP_GetCacheDataExact(PGXPVData* out, ushort index);

	/* attaches non-quantized UVs and shared tile bounds to the latest transform */
	void PGXP_SetLastTextureCoords(const float* uv, int count, const float* bounds);
