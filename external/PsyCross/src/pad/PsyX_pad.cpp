...
void PsyX_Pad_Event_ControllerRemoved(Sint32 deviceId)
{
	...
	// SDL_CONTROLLERDEVICEREMOVED carries a joystick instance ID, whereas the
	// added event and SDL_GameControllerOpen use a transient device index.
	// Compare against the opened joystick so a list reorder cannot disconnect
	// the wrong slot or leave this one holding a stale controller pointer.
	for (int i = 0; i < MAX_CONTROLLERS; i++)
	{
		controller = &g_controllers[i];
		if (controller->gc == NULL)
			continue;
		SDL_Joystick* joystick = SDL_GameControllerGetJoystick(controller->gc);
		if (joystick != NULL && SDL_JoystickInstanceID(joystick) == deviceId)
		{
...
void PsyX_Pad_UpdateGameControllerInput(SDL_GameController* cont, LPPADRAW pad)
{
	...
	if (!cont)
	{
		// PSX analog neutral is exactly 0x80. Using 0x7f here creates a
		// persistent off-centre sample for keyboard-only virtual pads.
		pad->analog[0] = 128;
		pad->analog[1] = 128;
		pad->analog[2] = 128;
		pad->analog[3] = 128;
...
{
	if (slot < 0 || slot >= MAX_CONTROLLERS || table == NULL || len <= 0)
		return;

	PsyXController* controller = &g_controllers[slot];
	if (controller->gc == NULL || !SDL_GameControllerGetAttached(controller->gc))
		return;
