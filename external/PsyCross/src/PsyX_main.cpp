...
int intrThreadMain(void* data)
{
	...
	while (!g_stopIntrThread)
	{
		// step counters
		{
			...
			// The old tight spin consumed an entire CPU core and regularly
			// pre-empted the presentation thread. Millisecond polling is precise
			// enough for the compatibility counter and leaves frame pacing alone.
			SDL_Delay(1);
		}
	}
...
static int PsyX_Sys_InitialiseCore()
{
	...
	// Publish the mutex before the worker can enter its first callback.
...
{
	int drawableWidth = g_windowWidth;
	int drawableHeight = g_windowHeight;
...
#ifdef _DEBUG
	if (g_cfg_vblankThread && nKey == SDL_SCANCODE_BACKSPACE)
	{
		if (down)
...
{
	static unsigned int sampleStart = 0;
	...
	if (sampleStart == 0)
		sampleStart = now;
	passedFrames++;
	const unsigned int elapsed = now - sampleStart;
	if (elapsed >= 1000)
	{
		currentFps = (passedFrames * 1000U + elapsed / 2U) / elapsed;
		passedFrames = 0;
		sampleStart = now;
	}
